...
SettingsTransfor.MaxValueCorr = 1.1; 
...
Init_A_coeff = .9*ones(4,1); 
Init_A_coeff = InvMaxZero(1,Init_A_coeff); 

Init_Omega = (eye(4)-diag(MaxZero(1,Init_A_coeff)))*initialF1; %c, initialF1 unconditional mean of f


Init_B_coeff_QQ = 0.015*ones(4,1); % 0.015; SQRT: .07
Init_B_coeff = InvMaxZero(.1,Init_B_coeff_QQ); 

Init_kap_hes = InvMaxZero(.5,.015);
%Init_Sigma_Meas_pd = InvPositiveTrans(.001); % meas. error

Init_phi1 = 0; %.3
Init_phi1 = InvMaxZero(.99,Init_phi1);  % ar coefficients

Init_psi1 = 0; %.9
Init_psi1 = InvMaxZero(.99,Init_psi1);  
...
LossToMinmize = @(vparam) -my_TVKF_pd_Pd_Model(vparam,y',initialF1,[],SettingsTransfor)/size(y,1);
